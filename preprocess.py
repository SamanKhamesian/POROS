...
def _compute_meal_frequency(day_df):
    """Number of meal events recorded by the pump (CarbSize > 0)."""
    return (day_df["CarbSize"] > 0).sum()
...
def _compute_first_meal_hour(day_df):
    """Hour of day (0-23) of the first meal event."""
    meal_times = day_df.loc[day_df["CarbSize"] > 0, "EventDateTime"].sort_values()
    return float(meal_times.iloc[0].hour) if not meal_times.empty else np.nan


def _compute_last_meal_hour(day_df):
    """Hour of day (0-23) of the last meal event."""
    meal_times = day_df.loc[day_df["CarbSize"] > 0, "EventDateTime"].sort_values()
    return float(meal_times.iloc[-1].hour) if not meal_times.empty else np.nan
...
def _compute_bolus_per_carb(day_df):
    """Ratio of food-bolus insulin to total pump-logged carbs."""
    total_carbs = day_df["CarbSize"].sum(skipna=True)
    bolus       = day_df["FoodDelivered"].sum(skipna=True)
    return float(bolus / total_carbs) if total_carbs > 0 else np.nan
...
def _compute_total_correction_insulin(day_df):
    ...
    """
    correction = day_df[
        (day_df["CarbSize"].fillna(0) == 0) &
        (day_df["InsulinDelivered"].notna()) &
        (day_df["InsulinDelivered"] > 0)
    ]
...
def _compute_avg_meal_bolus_delta_minutes(day_df):
    ...
    bolus_mask = (
        day_df["InsulinDelivered"].notna() &
        (day_df["InsulinDelivered"] > 0) &
        day_df["BolusType"].isin(FOOD_BOLUS_TYPES)
    )
...
def _compute_total_steps(day_df):
    return float(day_df["StepCount"].sum(skipna=True))


def _compute_exercise_duration(day_df):
    """Total exercise duration in minutes (5-min CGM intervals × non-null Exercise rows)."""
    return float(day_df["Exercise"].notna().sum() * 5)


def _compute_sleep_duration(day_df):
    """Total sleep duration in minutes (5-min intervals × non-null SleepStage rows)."""
    return float(day_df["SleepStage"].notna().sum() * 5)
...
def _prepare_daily_profile(df):
    ...
    for date, day_df in df.groupby("Date"):
        ...
        rows.append({
            ...
            "meal_frequency":           _compute_meal_frequency(day_df),
            "avg_carbs_per_meal":       _compute_avg_carbs_per_meal(day_df),
            "avg_time_between_meals":   _compute_avg_time_between_meals(day_df),
            "first_meal_hour":          _compute_first_meal_hour(day_df),
            "last_meal_hour":           _compute_last_meal_hour(day_df),

            # insulin features
            "total_daily_insulin":      _compute_total_daily_insulin(day_df),
            "bolus_per_carb":           _compute_bolus_per_carb(day_df),
            ...
            # activity / sleep
            "total_steps":        _compute_total_steps(day_df),
            "exercise_duration":  _compute_exercise_duration(day_df),
            "sleep_duration":     _compute_sleep_duration(day_df),
        })
...
def build_dataset(folder_path):
    ...
    for subject in os.listdir(folder_path):
        ...
        daily_df = daily_df[daily_df["sleep_duration"] >= 240].reset_index(drop=True)
...
if __name__ == "__main__":
    database = build_dataset(dataset_folder)
